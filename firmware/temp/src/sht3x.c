...
bool sht3x_probe(void) {
  ...
  if (!mgos_i2c_write(i2c, SHT31_ADDR, cmd, 2, false /* stop */)) {
    LOG(LL_ERROR, ("I2C write failed"));
    return false;
  }
  uint8_t resp[3] = {0};
  if (!mgos_i2c_read(i2c, SHT31_ADDR, resp, 3, true /* stop */)) {
  ...
  LOG(LL_INFO, ("Sensor status: 0x%02x%02x", resp[0], resp[1]));
  // Clear status.
  const uint8_t cmd2[2] = {0x30, 0x41};
  if (!mgos_i2c_write(i2c, SHT31_ADDR, cmd2, 2, true /* stop */)) {
    return false;
  }
...
void sht3x_read(float *temp, float *rh) {
  ...
  *rh = SHT31_INVALID_VALUE;
  ...
  if (!mgos_i2c_write(i2c, SHT31_ADDR, cmd, 2, true /* stop */)) {
    LOG(LL_ERROR, ("I2C write failed"));
    return;
  }
  uint8_t resp[6] = {0};
  mgos_msleep(12);  // Takes about 12 ms to perform the measurement.
  while (!mgos_i2c_read(i2c, SHT31_ADDR, resp, 6, true /* stop */)) {
